...
import { pintxos } from '../../../data/pintxos';
...
export default function PintxosPage() {
  ...
  const getLocalizedText = (text: string | Record<string, string | string[]>, locale: string): string | string[] => {
  ...
  const filteredAndSortedPintxos = useMemo(() => {
    const filtered = pintxos.filter(pintxo => {
      ...
      if (searchTerm) {
        ...
        const localizedDescription = getLocalizedText(pintxo.description, currentLocale) as string;
        const localizedIngredients = getLocalizedText(pintxo.ingredients, currentLocale) as string[];
        const localizedTags = getLocalizedText(pintxo.tags, currentLocale) as string[];
        
        const searchLower = searchTerm.toLowerCase();
        const matchesSearch = 
          localizedName.toLowerCase().includes(searchLower) ||
          localizedDescription.toLowerCase().includes(searchLower) ||
          (Array.isArray(localizedIngredients) && localizedIngredients.some(ingredient => 
            ingredient.toLowerCase().includes(searchLower))) ||
          (Array.isArray(localizedTags) && localizedTags.some(tag => 
      ...
      // Category filter
      if (filterByCategory && pintxo.category !== filterByCategory) return false;

      // Bar filter
      if (filterByBar && !pintxo.bars.includes(filterByBar)) return false;

      // Difficulty filter
      if (filterByDifficulty && pintxo.difficulty !== filterByDifficulty) return false;
    ...
    return filtered;
  }, [pintxos, searchTerm, sortBy, filterByCategory, filterByBar, filterByDifficulty, filterByPriceRange, currentLocale]);

  // Get unique bars for filter dropdown
  const uniqueBars = useMemo(() => {
    const barIds = new Set<string>();
    pintxos.forEach(pintxo => pintxo.bars.forEach(id => barIds.add(id)));
    return Array.from(barIds).map(id => ({ id, name: `Bar ${id.split('-').pop()}` }));
  }, [pintxos]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="bg-gradient-to-r from-red-600 to-red-700 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t('pintxos.title')}</h1>
          <p className="text-xl text-red-100 max-w-2xl mx-auto">{t('pintxos.subtitle')}</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-4 mb-6">
            ...
            <Select value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
              ...
              <SelectContent>
                <SelectItem value="popularity">{t('pintxos.popularity')}</SelectItem>
          ...
          <div className="flex flex-wrap gap-4 mb-6">
            <Select value={filterByCategory} onChange={(e) => setFilterByCategory(e.target.value)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="traditional">{t('pintxos.traditional')}</SelectItem>
                <SelectItem value="modern">{t('pintxos.modern')}</SelectItem>
                <SelectItem value="fusion">{t('pintxos.fusion')}</SelectItem>
                <SelectItem value="vegetarian">{t('pintxos.vegetarian')}</SelectItem>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filteredAndSortedPintxos.length > 0 ? (
            ...
            <div className="text-center py-16">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('pintxos.noResults.title')}</h2>
              <p className="text-lg text-gray-600 mb-8">{t('pintxos.noResults.description')}</p>
