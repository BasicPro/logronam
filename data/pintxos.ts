...
export const pintxos = [
  ...
  },
  {
    ...
    category: 'traditional' as const,
    ingredients: {
      es: ['pimientos del piquillo', 'bacalao', 'cebolla', 'tomate', 'aceite de oliva'],
      en: ['piquillo peppers', 'cod', 'onion', 'tomato', 'olive oil'],
      fr: ['poivrons piquillo', 'morue', 'oignon', 'tomate', 'huile d\'olive'],
      ca: ['pebrots del piquillo', 'bacallà', 'ceba', 'tomàquet', 'oli d\'oliva'],
      pt: ['pimentos piquillo', 'bacalhau', 'cebola', 'tomate', 'azeite de oliva'],
      de: ['Piquillo-Paprika', 'Kabeljau', 'Zwiebel', 'Tomate', 'Olivenöl'],
      it: ['peperoni piquillo', 'baccalà', 'cipolla', 'pomodoro', 'olio d\'oliva']
    },
  ...
  },
  {
    ...
    bars: ['bar-laurel-1', 'bar-laurel-2', 'bar-laurel-3', 'bar-laurel-4', 'bar-casco-1', 'bar-casco-2'],
    popularity: 5,
    difficulty: 'medium' as const,
    origin: {
      es: 'España',
  ...
  },
  {
    ...
    ingredients: {
      es: ['gambas', 'ajo', 'guindilla', 'perejil', 'aceite de oliva', 'vino blanco'],
      en: ['shrimp', 'garlic', 'chili', 'parsley', 'olive oil', 'white wine'],
      fr: ['crevettes', 'ail', 'piment', 'persil', 'huile d\'olive', 'vin blanc'],
      ca: ['gambes', 'all', 'bitxo', 'julivert', 'oli d\'oliva', 'vi blanc'],
      pt: ['camarão', 'alho', 'pimenta', 'salsa', 'azeite de oliva', 'vinho branco'],
      de: ['Garnelen', 'Knoblauch', 'Chili', 'Petersilie', 'Olivenöl', 'Weißwein'],
      it: ['gamberi', 'aglio', 'peperoncino', 'prezzemolo', 'olio d\'oliva', 'vino bianco']
    },
...
export function getPintxosByPriceRange(minPrice: number, maxPrice: number) {
  return pintxos.filter(pintxo => pintxo.price >= minPrice && pintxo.price <= maxPrice);
}
...
export function searchPintxos(query: string, locale: string = 'es') {
  ...
  return pintxos.filter(pintxo => 
    pintxo.name[locale as keyof typeof pintxo.name]?.toLowerCase().includes(lowercaseQuery) ||
    pintxo.description[locale as keyof typeof pintxo.description]?.toLowerCase().includes(lowercaseQuery) ||
    pintxo.ingredients[locale as keyof typeof pintxo.ingredients]?.some(ingredient => ingredient.toLowerCase().includes(lowercaseQuery)) ||
    pintxo.tags[locale as keyof typeof pintxo.tags]?.some(tag => tag.toLowerCase().includes(lowercaseQuery))
  );
}
