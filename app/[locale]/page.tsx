...
export default function Home() {
  ...
  const [topRatedBars, setTopRatedBars] = useState<Bar[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadBars = async () => {
      try {
        const bars = await getTopRatedBars(6, currentLocale);
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="text-center mt-12">
                <Link href={`/${currentLocale}/rankings`}>
                  <Button size="lg">{t("rankings.viewAllRankings")}</Button>
                </Link>
              </div>
            </>
          )}
