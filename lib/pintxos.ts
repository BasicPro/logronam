...
async function loadLocaleMap(locale: Locale): Promise<PintxoLocaleMap> {
  ...
  try {
    ...
    console.warn(
      `Failed to load locale ${locale}, falling back to ${DEFAULT_LOCALE}`
    );
 ...
 */
function merge(base: PintxoBase, translations: PintxoI18n): Pintxo {
  return {
    ...base,
    ...translations,
  };
...
export async function getPintxos(locale: Locale): Promise<Pintxo[]> {
  ...
  return pintxosBase.map((base) => {
    ...
    return merge(base, translations);
  });
}
 ...
 */
export async function getPintxosByCategory(
...
export async function getPintxosByPriceRange(
  ...
  const allPintxos = await getPintxos(locale);
  return allPintxos.filter(
    (pintxo) => pintxo.price >= min && pintxo.price <= max
  );
}
...
export async function searchPintxos(
  ...
  return allPintxos.filter((pintxo) => {
    return (
      ...
      pintxo.tags.some((tag) => tag.toLowerCase().includes(lowerQuery))
    );
  });
}
